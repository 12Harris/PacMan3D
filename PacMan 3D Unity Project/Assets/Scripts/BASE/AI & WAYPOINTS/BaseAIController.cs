...
namespace GPC
{
	...
	public class BaseAIController : ExtendedCustomMonoBehaviour
	{
		...
		//In 3d units, how far the bot should look ahead in its attempts to avoid running into obstacles when it is patrolling.
		...
		//In 3d units, minChaseDistance determines how close the bot should get to its target, when chasing a target, before it stops moving toward it.
		...
		public virtual int IsObstacleAhead()
		{
			...
			if (is2DGame)
			{
				UnityEngine.Debug.DrawRay(_TR.position, ((_TR.up + (-_TR.right * 0.5f)) * obstacleAvoidDistance));
				UnityEngine.Debug.DrawRay(_TR.position, ((_TR.up + (_TR.right * 0.5f)) * obstacleAvoidDistance));
			}
			...
			else
			{
				...
				UnityEngine.Debug.DrawRay(_TR.position, ((_TR.forward + (-_TR.right * 0.5f)) * obstacleAvoidDistance));
				UnityEngine.Debug.DrawRay(_TR.position, ((_TR.forward + (_TR.right * 0.5f)) * obstacleAvoidDistance));
			}
		...
		public bool CanSee(Transform aTarget)
		{
			...
			UnityEngine.Debug.DrawLine(_TR.position + (visionHeightOffset * _TR.up), aTarget.position, Color.red);
